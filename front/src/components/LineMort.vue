<template>
  ...
  padding-left: 8%;">
    ...
    <table>
      ...
      <tr>
        <th>Année</th>
        <th>Nombre de personne mort a cause du diabéte</th>
      </tr>
      </thead>
      ...
      <tr v-for="(ligne, index) in lines" :key="index">
        ...
        <td data-title="Id">{{ligne.total_morts}}</td>
      </tr>
      </tbody>
...
export default {
  ...
  data: () => ({
    ...
    chartOptions: {
      ...
      title: {
        text: "Population mondiale morte de diabète depuis l'an 2000",
        margin: 50,
        style: {
          color: '#000000',
          fontWeight: 'bold',
      ...
      xAxis: {
        accessibility: {
          rangeDescription: 'Range: 2000 to 2045'
        }
      },
      yAxis: {
        title: {
  ...
  methods: {
    ...mapActions(["getLine",'getInfoLineMort']),
    toggleTable() {
  ...
  mounted() {
    this.getLine()
        .then(() => {
          ...
          let data = [];
          for (let year = 2000; year <= 2045; year++) {
            let found = false;
            this.lines.forEach((item) => {
              if (item.annee === year) {
                data.push({
                  name: item.iso_pays_car,
                  y: parseFloat(item.total_morts),
                  color: "orange",
                });
                found = true;
              }
          ...
          this.chart.setTitle({
            text:
                "Population mondiale morte de diabète depuis l'an 2000"
          });
