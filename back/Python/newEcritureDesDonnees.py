...
def convertirSexe(a, genreActuel):
   if (genreActuel == 'men'):
      return 1
   else:
      return 0
...
def calculModification(donnees, ligne_existante):
    with open('report_diabetique.csv', 'r', newline='') as fichier_mort:
...
def donneesDejaPresentes(donnees):
    try:
        with open('report_diabetique.csv', "r", newline='', encoding='utf-8') as fichier:
        ...
        print("Le fichier 'report_diabetique.csv' n'a pas été trouvé.")
        return False
    except Exception as e:
...
def modifierDonnees(donnees, ligne_existante):
    ...
    with open('report_diabetique.csv', 'w', newline='') as fichier_mort:
        writer = csv.writer(fichier_mort)
        writer.writerows(lignes)

#--------------------------------------------------
#Utilité: Faire un produit en croix (en gros) selon les besoins de l'utilisateur
#Paramètres: Les données à modifier ainsi que le ratio (ex: 1 pour 100)
#Retour: Un tableau qui a pour seul changement les nouvelles données
def ratio(nouvellesDonnees, a, b):
   ...
   for row in cr:
      ...
      for i in range(car):
         if ligne[0][i] != "," and ligne[0][i] != ";":
            ...
            txt = ""
      tableau.append(txt)
      if str(nouvellesDonnees[0]) == tableau[0].upper() and str(nouvellesDonnees[1]) == tableau[1].upper():
         #print(nouvellesDonnees[0] , "-" , nouvellesDonnees[1], " ---------", tableau[0] , "-" , tableau[1])
         nouvellesDonnees[2] = round((float(nouvellesDonnees[2]) * a) * float(tableau[2]) / b,2)
...
#Retour: Aucun
def ecrireDonneesSiMort(n, fichier_csv, tabN, r1, r2):
   fichier = open(fichier_csv,"r")
   fichier_mort = open('report_diabetique.csv', 'a', newline='')
   write = csv.writer(fichier_mort)
   i = 0
   for ligne in fichier:
      if i > n:
         tab = (ligneN(fichier_csv, i))
         pays = convertirPays('PAYS.csv', tab[tabN[0]], tab, tabN)
         sexe = convertirSexe('motscles_homme.txt', tab[tabN[3]])
         if pays != False and str(pays).isnumeric():
            nouvellesDonnees = [pays, tab[tabN[1]], tab[tabN[2]], sexe]
            if (r1 != 1 or r2 != 1):
               fhehjnlsqzjdkùbahibdjoqshkodzhaio = ratio(nouvellesDonnees, r1, r2)
               if (fhehjnlsqzjdkùbahibdjoqshkodzhaio != False):
                  nouvellesDonnees = fhehjnlsqzjdkùbahibdjoqshkodzhaio
...
#Retour: Aucun
def ecrireDonneesSiAnnees(n, fichier_csv, tabN, tabIndex, tabIndexGlobal, tabAnnees, ligne1, r1, r2):
   fichier_mort = open(fichier_csv,"r")
   fichier_mort_writer = open('report_diabetique.csv', 'a', newline='')
         ...
         if pays != False and str(pays).isnumeric():
            for j in range(len(tabIndex)):
               if est_nombre(tab_fichier[tabIndexGlobal[j]]):
                  nouvellesDonnees = [pays,tabAnnees[tabIndex[j]],tab_fichier[tabIndexGlobal[j]]]
                  if (r1 != 1 or r2 != 1):
                     fhehjnlsqzjdkùbahibdjoqshkodzhaio = ratio(nouvellesDonnees, r1, r2)
                     if (fhehjnlsqzjdkùbahibdjoqshkodzhaio != False):
                        nouvellesDonnees = fhehjnlsqzjdkùbahibdjoqshkodzhaio
...
def prmg(r1 = 0.01, r2 = 1):
   ...
   if (nColonneAnnees != False):
      motscles_diabete = motClesToTab('motscles_diabete.txt')
      nColonneMort = chercheNomColonne(tab, motscles_diabete)
      motscles_sexe = motClesToTab('motscles_sexe.txt')
      nColonneSexe = chercheNomColonne(tab, motscles_sexe)
      tabColonnes = [nColonnePays, nColonneAnnees, nColonneMort, nColonneSexe]
      ecrireDonneesSiMort(nLigne1, fichier, tabColonnes, r1, r2)
      return True
   else:
      tabIndex, tabAnnees, tabIndexGlobal = chercheAnnees(tab)
      tabColonnes = [nColonnePays]
      ecrireDonneesSiAnnees(nLigne1, fichier, tabColonnes, tabIndex, tabIndexGlobal, tabAnnees, tab, r1, r2)
      return True
